...
export type ActionsFavouritesType = InferActionsTypes<typeof actions>
type ThunkType = BaseThunkType<ActionsFavouritesType>

const favouritesReducer = (
  state = initialState,
  action: ActionsFavouritesType
): InitialStateType => {
  switch (action.type) {
...
export const getFavouritesList =
  (limit = 5, page = 0, fetch = true): ThunkType =>
  async (dispatch) => {
...
export type getFavouritesListThunkType = typeof getFavouritesList
