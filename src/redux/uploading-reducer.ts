...
const uploadingReducer = (state = initialState, action: ActionsType): InitialStateType => {
   switch (action.type) {
...
export const actions = {
   toggleIsUploading: (isUploading: boolean) => ({ type: TOGGLE_IS_UPLOADING, isUploading } as const),
   toggleIsFetching: (isFetching: boolean) => ({ type: TOGGLE_IS_FETCHING, isFetching } as const),
   setCurrentFile: (currentFile: File | null) => ({ type: SET_CURRENT_FILE, currentFile } as const),
   setFileName: (fileName: string) => ({ type: SET_FILE_NAME, fileName } as const),
   setFilePath: (filePath: string) => ({ type: SET_FILE_PATH, filePath } as const),
   toggleIsCat: (isCat: boolean | null) => ({ type: TOGGLE_IS_CAT, isCat } as const),
}

export const uploadImage = (file: File): ThunkType => async (dispatch) => {
...
export const setCurrentFile = (target: any): ThunkType => async (dispatch) => {
